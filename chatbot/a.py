...
# Singleton pour le modèle spaCy
...
class RecommandationIA:
    ...
    def _init_skill_patterns(self):
        """Initialise les modèles de compétences pour le PhraseMatcher"""
        raw_skills = [
    ...
    def get_groq_embeddings(self, text: str) -> np.ndarray:
        """Génère des embeddings avec l'API Groq"""
        ...
        try:
            response = self.groq_client.embeddings.create(
                input=[text],
                model="llama3-8b-8192"  # Modèle mis à jour
            )
    ...
    def calculer_score(self, texte_offre: str, texte_etudiant: str) -> float:
        """Calcule un score combiné utilisant plusieurs méthodes"""
        ...
        try:
            ...
            # Similarité avec embeddings Groq
            ...
            return round(0.4 * score_spacy + 0.3 * score_tfidf + 0.3 * score_groq, 3)
        except Exception as e:
            print(f"Erreur dans le calcul de similarité : {e}")
            return 0.0

    def extraire_texte_pdf(self, fichier_pdf) -> str:
        """Extraction robuste du texte PDF"""
    ...
    def extraire_competences(self, texte: str) -> List[str]:
        """Extraction des compétences avec NER et pattern matching"""
        if not texte:
            return []
    ...
    def apply_hybrid_recommendation(self, etudiant_id: int, content_based: List[Dict]) -> List[Dict]:
        """Combine content-based et collaborative filtering"""
        try:
            ...
            # Correction: utilisation de 'candidature' au lieu de 'candidatures'
    ...
    def analyser_points_forts(self, texte_offre: str, texte_etudiant: str) -> List[str]:
        """Identifie les points forts de correspondance"""
    ...
    def expliquer_recommandation(self, offre_id: int, etudiant_id: int) -> str:
        """Génère une explication détaillée de la recommandation avec Groq"""
        ...
        try:
            ...
            # Génération de l'explication avec Groq
            prompt = (
    ...
    def update_user_preferences(self, user_id: int, accepted_offers: List[int], rejected_offers: List[int]):
        """Met à jour les préférences de l'utilisateur basé sur ses actions"""
        try:
            etudiant = Etudiant.objects.get(user_id=user_id)
            
            # Analyse des offres acceptées
            ...
            # Mise à jour des weights pour le calcul de score
...
class ChatbotService:
    ...
    def _generate_groq_response(self, prompt: str, max_tokens: int = 200) -> str:
        """Utilise Groq pour générer une réponse plus naturelle"""
    ...
    def _setup_intents(self):
        """Configuration des intentions et du matcher"""
    ...
    def _detect_intent(self, message: str) -> Tuple[str, float]:
        """Détection d'intention améliorée avec contexte"""
        ...
        if matches:
            ...
            # Gestion des intentions contextuelles
            if intent == "DETAILS" and last_intent == "OFFERS":
                return "OFFER_DETAILS", 1.0
                
            return intent, 1.0
        
        # Vérification des intentions simples
    ...
    def _generate_response(self, intent: str, user, confidence: float) -> str:
        """Génération de réponse améliorée avec Groq"""
        try:
    ...
    def process_message(self, user, message: str) -> str:
        """Traitement complet du message avec historique"""
